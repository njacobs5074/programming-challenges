...
class HittableList(val objects: Vector[Hittable]) extends Hittable:
  /**
   * Iterate over the collection of Hittable objects and figure out which one, if any,
   * is struck by the supplied ray in the interval [tMin, tMax).
   */
  override def hit(ray: Ray, tMin: Double, tMax: Double): Option[HitRecord] =
    ...
    objects.foldLeft[(Option[HitRecord], Double)]((None, tMax)) { case ((hitObj, closestSoFar), obj) =>
